...
def is_divisible_by_three(isit):
    if isit % 3 == 0:
        return True
    else: 
        return False
...
def total_meal_price(meal_price, tip=None): # Adding =None at the end of the second parameter makes 
                                            # it a default value. It won't cause an error if only one
                                            # value is passed.

    if tip is None:
        tip = 0.15  # Set default tip to 15%

    total = meal_price + meal_price * tip # Otherwise, if there is a 2nd parameter added, this will be the equation
...
nombre = -42
results = sign_and_parity(nombre)
sign = results[0]
parity = results[1]
...
def write_letter(recipient_name, job, sender_name):
    for_recipient = full_title(recipient_name, job)
    print(f"Dear {for_recipient}, I think you are amazing! Sincerely, {sender_name}")
